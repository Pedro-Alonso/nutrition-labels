...
export function RiskSummary({ productName, barcode, riskLevel, passed }: RiskSummaryProps) {
  const name = productName ?? 'Produto desconhecido';

  return (
    <View className={`${getRiskBgClass(riskLevel)} px-4 pt-5 pb-6`}>
      <View accessible accessibilityLabel={`Produto analisado: ${name}. Risco global: ${RISK_LABELS[riskLevel]}`}>
